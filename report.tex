...
VADER, as a non-transformer, is used as a baseline against which to evaluate the transformer models. In this study, VADER is not trained, but is used to provide baseline classification scores by being run on a concatenation of all the datasets and evaluating its performance. 
...
VADER performs the worst of all the models, which is expected as it does not follow the transformer architecture and because VADER is not trained on the Twitter data before testing, as the transformer models are. 
